...
from cdsvae.utils import KL_divergence, inception_score, entropy_Hy, entropy_Hyx
from sample_and_predict import contrastive_loss
...
def arg_parse():
  ...
  parser.add_argument("--dataset_path", default='', help='set you own dataset path')
...
def main(args):
  ...
  cdsvae = torch.load("./weights/model.pth")
  ...
  batch = utils.reorder(next(iter(test_loader))['images'])
  batch = batch.cuda()
  cdsvae.eval()
  swapped_batch = cdsvae.swap(batch)

  x1 = batch[0].unsqueeze(dim=0)
  x2 = batch[1].unsqueeze(dim=0)
  x1_s_x2_d = swapped_batch[0].unsqueeze(dim=0)
  x1_d_x2_s = swapped_batch[1].unsqueeze(dim=0)

  utils.imshow_seqeunce([x1, x2, x1_s_x2_d, x1_d_x2_s])
